...
fb22 <- function(ba,  bb, Afun, afun,  bg, x, t){
    ...
    b2 <- function(i){
        ...
        c(integral(integrand1, 0.001, mtau,  method = "Simpson", vectorized = T, arrayValued = T, waypoints = NULL, reltol = 1e-08, abstol = 0, i))
    }
    t(do.call(rbind, lapply(1 : m, b2)))
}
fA22 <- function(ba, bb, Afun, afun, bg, br, x, t){
    ...
    A2 <- function(ij){
        ...
        res <- c(integral(vintegrand, 0.001, mtau,  method = "Simpson", vectorized = T, arrayValued = T, waypoints = NULL, reltol = 1e-08, abstol = 0, i, j))
...
fmb <- function(i, mt, bb, A, a, bg, br){
    ...
    if(t <= 2){
        temp <- BBoptim(c(A(t), a(t)), tryscorealphasum, gr= NULL, method=1, lower=c(0, 0.001), upper=c(2, 2), project=NULL, projectArgs=NULL, control=list(maxit = 30, ftol = 1e-8, gtol = 1e-5), quiet=FALSE,  bb, data, A, a, bg, br, t, me2)#dfsane((c(A(t) , a(t))), dscorealphasum, method = 2, control = list(tol = 1e-5), quiet = FALSE,  bb, data, A, a, bg, br, t, me2)##dfsane((c(A(t), a(t))),tryscorealphasum, method = 2, control = list(tol = 1e-5), quiet = FALSE,  bb, data, A, a, bg, br, t, me2)#####m## #### optim(c(A(t), a(t)) , tryscorealphasum, gr = NULL, bb, data, A, a, bg, br, t, me2, method = "L-BFGS-B", lower = c(0, 0.001), upper = c(2, 2), control = list(), hessian = FALSE)# ####
    ...
    temp <- BBoptim(c(A(t), a(t)), tryscorealphasum, gr= NULL, method=1, lower=c(0, 0.001), upper=c(30, 8), project=NULL, projectArgs=NULL, control=list(maxit = 30, ftol = 1e-8, gtol = 1e-5), quiet=FALSE,  bb, data, A, a, bg, br, t, me2)#dfsane((c(A(t) , a(t))), dscorealphasum, method = 2, control = list(tol = 1e-5), quiet = FALSE,  bb, data, A, a, bg, br, t, me2)##dfsane((c(A(t), a(t))),tryscorealphasum, method = 2, control = list(tol = 1e-5), quiet = FALSE,  bb, data, A, a, bg, br, t, me2)#####m## #### optim(c(A(t), a(t)) , tryscorealphasum, gr = NULL, bb, data, A, a, bg, br, t, me2, method = "L-BFGS-B", lower = c(0, 0.001), upper = c(2, 2), control = list(), hessian = FALSE)# ####
        print(temp$convergence)
        print(temp$value)
        print(temp$gradient)
        print(temp$message)
...
for(itr  in sta:(sta + 99)){
    ...
    lmt <- 10
   # mt <- c(seq(quantile(data[data[, 1] == 1, 2], 0.1), quantile(data[data[, 1] == 1, 2], 0.8), length.out = lmt))
