...
fb2 <- function(A, a,  bb, bg, x, t){
    expbbx <- exp(matrix(x, nrow = 1) %*% matrix(bb, ncol = 1))
...
fb21 <- function(ba,  bb, Afun, afun,  bg, x, t){
    expbbx <- exp(x * bb)#exp(matrix(x, nrow = 1) %*% matrix(bb, ncol = 1))
    integrand1 <- function(s, i){
...
dscorealphasum <- function(ba, bb, data, A, a, bg, br, t, me2){
    ...
    e2 <- t(ginv(t(A2)%*% A2) %*% (t(A2) %*% t(b2)))
    me2[[1]] <- e2
    A2 <- fA21(ba, bb, A, a, bg, br, 1.5, t)
    b2 <- fb21(ba, bb, A, a, bg, 1.5, t)
    e2 <- t(ginv(t(A2)%*% A2) %*% (t(A2) %*% t(b2)))
    me2[[2]] <- e2
    vsum <- apply(do.call(cbind, mclapply(1 :n,  dscorealpha, data, ba, bb, A, a, bg, br, t, me2, mc.cores = 20)), 1, sum,  na.rm = T) #* (oba)
    return(vsum)
#    sqrt(t(vsum) %*% vsum)
    #mean(do.call(rbind, lapply(1 :n, scorealpha, data,   ba, bb, A, a, bg, br, t, me2)))
}
varalphasum <- function(ba, bb, data, A, a, bg, br, t, me2){
    #print(ba)
    ## if(i > 1){
    ##     oba <- exp(ba)
    ##     ba <- exp(ba) + c(exp((mba[i - 1, 1])), 0)
    ...
    e2 <- t(ginv(t(A2)%*% A2) %*% (t(A2) %*% t(b2)))
    me2[[2]] <- e2
    temp <- (do.call(cbind, mclapply(1 :n,  dscorealpha, data, ba, bb, A, a, bg, br, t, me2, mc.cores = 20)))
    temp %*% t(temp)
   # return(vsum)
#    sqrt(t(vsum) %*% vsum)
    #mean(do.call(rbind, lapply(1 :n, scorealpha, data,   ba, bb, A, a, bg, br, t, me2)))
...
fmb <- function(i, mt, bb, A, a, bg, br){
    ...
    if(t <= 2){
         temp <- spg(c(iniA(t), inia(t)), tryscorealphasum, gr= NULL, method=1, lower=c(0, 0.001), upper=c(2, 2), project=NULL, projectArgs=NULL, control=list(maxit = 30, ftol = 1e-8, gtol = 1e-5), quiet=FALSE,  alertConvergence = TRUE, bb, data, A, a, bg, br, t, me2)#dfsane((c(A(t) , a(t))), dscorealphasum, method = 2, control = list(tol = 1e-5), quiet = FALSE,  bb, data, A, a, bg, br, t, me2)##dfsane((c(A(t), a(t))),tryscorealphasum, method = 2, control = list(tol = 1e-5), quiet = FALSE,  bb, data, A, a, bg, br, t, me2)#####m## #### optim(c(A(t), a(t)) , tryscorealphasum, gr = NULL, bb, data, A, a, bg, br, t, me2, method = "L-BFGS-B", lower = c(0, 0.001), upper = c(2, 2), control = list(), hessian = FALSE)# ####
    ...
    temp$par
        ...
        temp <- spg(c(iniA(t), inia(t)), tryscorealphasum, gr= NULL, method=1, lower=c(0, 0.001), upper=c(30, 8), project=NULL, projectArgs=NULL, control=list(maxit = 30, ftol = 1e-8, gtol = 1e-5), quiet=FALSE,  alertConvergence = TRUE, bb, data, A, a, bg, br, t, me2)#dfsane((c(A(t) , a(t))), dscorealphasum, method = 2, control = list(tol = 1e-5), quiet = FALSE,  bb, data, A, a, bg, br, t, me2)##dfsane((c(A(t), a(t))),tryscorealphasum, method = 2, control = list(tol = 1e-5), quiet = FALSE,  bb, data, A, a, bg, br, t, me2)#####m## #### optim(c(A(t), a(t)) , tryscorealphasum, gr = NULL, bb, data, A, a, bg, br, t, me2, method = "L-BFGS-B", lower = c(0, 0.001), upper = c(2, 2), control = list(), hessian = FALSE)# ####
       # print(temp$convergence)
       # print(temp$value)
       # print(temp$gradient)
       # print(temp$message)
...
fb12 <- function(A, a, bb, bg,br,  x){
    ...
    t(do.call(rbind, lapply(1 : m1, b1)))
}
...
varbetasum <- function( bb,  data, A, a, bg, br){
   ##  obb <- bb
   ##  expbb<-  exp(bb)/(1 + exp(bb))
   ##  if(is.nan(expbb)){
   ##      expbb <- 1
   ...
   b1 <- fb12(A, a, bb, bg, 0.5)
   me1[[2]] <- t(ginv(t(A1)%*% A1) %*% (t(A1) %*% t(b1)))
   temp <- do.call(cbind, mclapply(1 :n, dscorebeta, data,  bb, A, a, bg, br, me1, mc.cores = 20))
   return(temp %*% t(temp))
...
fmbeta <- function( data, bb, A, a, bg, br){
 ...
 #temp <- dfsane(bb, dscorebetasum, method = 2, control = list(maxit = 30), quiet = FALSE, data, A, a, bg, br)#
   temp <- spg(bb, tryscorebetasum, gr= NULL, method=3, lower= -1, upper=2, project=NULL, projectArgs=NULL, control=list(maxit = 15, ftol = 1e-6, gtol = 0.0001), quiet=FALSE, alertConvergence = TRUE,   data, A, a, bg, br)#optim(0.5, tryscorebetasum, gr = NULL, data, A, a, bg1, br1,   method = "L-BFGS-B", lower = -1, upper = 2, control = list(maxit = 30, REPORT = 1, pgtol = 1e-5, trace = 0), hessian = FALSE)#### 
...
varbetafun <- function(bb, A, a, data, bg, br){
    
    derivscore <- jacobian(dscorebetasum, bb, method = "simple",  method.args = ls(), data, A, a, bg, br)
    B <- varbetasum( bb,  data, A, a, bg, br)
    invderiv <- ginv(derivscore)
    invderiv %*% B %*% t(invderiv)
    }
varalphafun <- function( bb, data, A, a, bg, br, t, me2){
    
    derivscore <- jacobian(dscorealphasum, c(A(t), a(t)), method = "simple",  method.args = ls(), bb, data, A, a, bg, br, t, me2)
    B <- varalphasum(c(A(t), a(t)), bb,  data, A, a, bg, br, t, me2)
    invderiv <- ginv(derivscore)
    invderiv %*% B %*% t(invderiv)
}
####test code###########
...
for(itr  in sta:(sta + 0)){
    ...
    bg1 <- seq(0.1, 5, length.out = m1)# seq(qgamma(0.002, 1/v, 1/v), qgamma(0.998, 1/v, 1/v), length.out = m)#as.vector(quantile(rg, seq(0, 1, length.out = m)))
    br1 <- rep(1, m1) / m1 #dgamma(bg, 1/v, 1/v)/ sum(dgamma(bg, 1/v, 1/v))#dnorm(bg, mean(rg), sd(rg))/sum(dnorm(bg, mean(rg), sd(rg)))##rep(1, 30)/30#
    n <- nrow(data)
    h <- bw.nrd(data[data[, 1] == 1, 2]) * n^{-1/15}
    mij <- as.matrix(expand.grid(1:m, 1:m))
    mij1 <- as.matrix(expand.grid(1:m1, 1:m1))
    lmt <- 20
...
mbb <- try(fmbeta(data, obb, A, a, bg1, br1))
...
#save(mba, mbb, trueA, truea,  file = paste(paste("../res/", itr, sep = ""),  obb, "res", sep = "_"))
}
